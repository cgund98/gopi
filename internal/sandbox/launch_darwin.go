...
// Launch runs argv under sandbox-exec. The child is its own process group.
func Launch(ctx context.Context, profile Profile) (Result, error) {
	...
	cmd := exec.CommandContext(runCtx, "/usr/bin/sandbox-exec", append([]string{"-p", policy}, profile.Argv...)...)
